...
int main(void)
{
	...
	while(1)
	{
		...
		if(tlinkUartFlag != 0)
		{
				GizWits_GatherSensorData();
				//发送红外			
				UART3_Send_DATA(ReadTypeDef.Infrared);
				//发送温度		
				UART3_Send_DATA(ReadTypeDef.Temperature-13);
				//发送湿度		
				UART3_Send_DATA(ReadTypeDef.Humidity);
				tlinkUartFlag = 0;
		}
